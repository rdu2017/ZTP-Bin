...
Simu_Ui <- read.table('Simu_Ui.txt', header=T, stringsAsFactors = F) # read in the file containing Ui(s) to each COG
...
Simu_blaBH <- read.table("Simu_blastBH.txt", header=T) # read in the file containing alignment scores, and to which COG the read is trully associated 
...
HOT25_crtCount <- read.table('HOT25_crtCount.txt', header=T) # read in a real dataset which has gone through Step 1&2
